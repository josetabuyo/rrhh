---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- =============================================
-- Author: Javier y belen
-- ALTER  date: 13/09/2012
-- Description:	Trae un viático
-- =============================================

ALTER PROCEDURE [dbo].[VIA_GetViaticoPorId]
@Id_Viatico int 
AS
BEGIN
	SET NOCOUNT ON;
 ...
 FROM  VIA_ComisionesDeServicio CS   
    ...
    LEFT JOIN dbo.tabla_areas TAP ON    
     TAP.id = DA.id_area         
    LEFT JOIN dbo.tabla_areas_detalle TADD on 
    TADD.id_area = TAP.id     
