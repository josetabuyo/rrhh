...
var AdministradorPlanillaCursos = function () {
    ...
    panelCurso.CompletarDatosCurso = function (un_curso) {
        ...
        panelCurso.find("#txtHorario").val(un_curso.Horarios);
...
$('#cmbMateria').change(function () {
    $('#txtIdMateria').val($('#cmbMateria').val());
    $('#txtNombre').val($('#cmbMateria').val());
});
...
var ValidarSuperposicion = function () {
    ...
    $.each(horarios, function (index, horario) {
        if (horario.NumeroDia == horario_a_validar.NumeroDia) {
            if (parseInt(horario_a_validar.HoraDeInicio.replace(regExp, "$1$2")) >= parseInt(horario.HoraDeInicio.replace(regExp, "$1$2")) &&
            parseInt(horario_a_validar.HoraDeInicio.replace(regExp, "$1$2")) < parseInt(horario.HoraDeFin.replace(regExp, "$1$2"))) {
                res = false;
            } if (parseInt(horario_a_validar.HoraDeFin.replace(regExp, "$1$2")) > parseInt(horario.HoraDeInicio.replace(regExp, "$1$2")) &&
            parseInt(horario_a_validar.HoraDeFin.replace(regExp, "$1$2")) <= parseInt(horario.HoraDeFin.replace(regExp, "$1$2"))) {
                res = false;
            } if (parseInt(horario_a_validar.HoraDeInicio.replace(regExp, "$1$2")) <= parseInt(horario.HoraDeInicio.replace(regExp, "$1$2")) &&
            parseInt(horario_a_validar.HoraDeFin.replace(regExp, "$1$2")) >= parseInt(horario.HoraDeFin.replace(regExp, "$1$2"))) {
                res = false;
    ...
    if (!res)
        alert("El horario que intenta agregar se superpone con otros horarios de la lista");
    return res;
}

var ValidarRangoDeHoras = function (hora_inicio, hora_fin) {
    var regExp = /(\d{1,2})\:(\d{1,2})/;
    if (parseInt(hora_inicio.replace(regExp, "$1$2")) < parseInt(hora_fin.replace(regExp, "$1$2")))
        return true;
    else {
