...
var ValidarSuperposicion = function () {
    ...
    $.each(horarios, function (index, horario) {
        if (horario.NumeroDia == horario_a_validar.NumeroDia) {
            ...
            parseInt(horario_a_validar.HoraDeFin.replace(regExp, "$1$2"), 10) <= parseInt(horario.HoraDeFin.replace(regExp, "$1$2"), 10)) {
                res = false;
            } if (parseInt(horario_a_validar.HoraDeInicio.replace(regExp, "$1$2"), 10) <= parseInt(horario.HoraDeInicio.replace(regExp, "$1$2"), 10) &&
            parseInt(horario_a_validar.HoraDeFin.replace(regExp, "$1$2"), 10) >= parseInt(horario.HoraDeFin.replace(regExp, "$1$2"), 10)) {
                res = false;
...
var ValidarRangoDeHoras = function (hora_inicio, hora_fin) {
    var regExp = /(\d{1,2})\:(\d{1,2})/;
    if (parseInt(hora_inicio.replace(regExp, "$1$2"), 10) < parseInt(hora_fin.replace(regExp, "$1$2"), 10))
        return true;
    else {
