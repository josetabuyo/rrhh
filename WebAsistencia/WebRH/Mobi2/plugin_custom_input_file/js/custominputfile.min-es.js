...
(function ($) {
    CustomFile = function (elem, options) {
        ...
        this.defaults = {
            ...
            allowed: 'all',
            notAllowed: [],
            addContainerAfter: $(elem),
            multiple: true,
            maxFiles: 5,
            maxMB: 0,
            maxKBperFile: 2048,
            fileWrapper: '<div class="cif-parent kv-file-content" style="margin: 0 auto;width: 150px;padding-top: 0px;"></div>',
            //filePicker: '<h3>Soltar archivos aquí</h3><p>o click para seleccionar desde carpeta</p><div class="cif-icon-picker"></div>',
            name:'Subir Foto',
            filePicker: '<span class="btn btn-primary btn-file"><i class="glyphicon glyphicon-picture"></i>&nbsp;'+options.name+'</span>',
            image: {
                crop: false,
                preview: {
                    display: true,
    ...
    CustomFile.prototype = {
        init: function () {
            ...
            //this.container = $('<div class="file-preview-frame krajee-default  kv-preview-thumb"  data-template="image"><div class="kv-file-content">');
        ...
        setFileWrapper: function () {
            ...
            this.settings.fileWrapper = $('<div>', {
                class: "cif-file-row file-preview-frame krajee-default  kv-preview-thumb"
            }).append(this.settings.fileWrapper);
            var fwr = this.settings.fileWrapper;
            fwr.attr("style", "padding: 6px 7px;margin: 2px;");
            ...
            if (fwr.find('.cif-parent').length == 0) {
                ...
                //fwr.prepend('<div class="cif-parent file-preview-frame krajee-default  kv-preview-thumb" data-template="image"><div class="kv-file-content">');
            }
        },
    ...
    FilePicker = function (app) {
        this.btn = $('<span class="cif-file-pickerxxx"></span>').append(app.settings.filePicker);
        this.init(app);
    }
    FilePicker.prototype = {
        init: function (app) {
            ...
            inputHidden.change(function () {
                ...
                if (app.status == 'reading') return $.customFile.popup.add(app.settings.messages.errorOnReading, popupSet.autoclose, popupSet.delay, 'error');
            ...
            btn.on({
                ...
                drop: function (e) {
                    ...
                    if (fileList.length > 1 && !app.settings.multiple) return $.customFile.popup.add(app.settings.messages.errorMultipleDisable, popupSet.autoclose, popupSet.delay, 'error');
                    app.status = 'reading';
                    app.read(fileList, 0);
                }
            });
    ...
    FileItem.prototype = {
        ...
        readImage: function (callback) {
            ...
            fileObj.img.onload = function () {
                ...
                /////
                imgNode.attr("class", "file-preview-image kv-preview-data"  );
                imgNode.attr("style", "width: 100%; max-width: 100%; image-orientation: from-image; padding-top: 0px;");  
                
                /////fileObj.node.find("img.cif-img")
                //imgNode.parent().prepend(fileObj.btnClose);
                //imgNode.parent().prepend('<div>');
                
                app.container.append(fileObj.node);
                if (app.settings.image.crop === true) {
                    fileObj.jcropObj = fileObj.initJcrop(app.settings.image, imgNode.parent(), fileObj.img, app.name);
                }
                /////
                var parent = fileObj.node.find('.cif-parent').parent();
                var fileSize = (fileObj.file.size < 102400) ? (fileObj.file.size / 1024).toFixed(2) : Math.round(fileObj.file.size / 1024);
                fileObj.btnClose = $('<button style ="margin: 0px 3px;" type="button" class="kv-file-remove btn btn-sm btn-kv btn-danger" title="Eliminar" ><i class="fas fa-trash-alt"></i></button>');
                fileObj.btnVer = $('<button style ="margin: 0px 3px;" type="button" class="kv-file-zoom btn btn-sm btn-kv btn-default btn-outline-secondary" title="Ver"><i class="fas fa-search-plus"></i></button');
                parent.append('<div class="file-thumbnail-footer"><div class= "file-footer-caption" title = "' + fileObj.file.name + '" ><div class="file-caption-info">' + fileObj.file.name + '</div> <div class="file-size-info"> <samp>(' + fileSize + ' KB)</samp></div> </div > <div class="file-actions"> <div class="file-footer-buttons" style="margin-bottom: 0px;"> </div>  </div>  <div class="clearfix"></div></div >');
                fileObj.node.find('.file-footer-buttons').append(fileObj.btnClose);
                fileObj.node.find('.file-footer-buttons').append(fileObj.btnVer);
                //parent.append('<button type="button" class="kv-file-zoom btn btn-sm btn-kv btn-default btn-outline-secondary" title="View Details"><i class="fas fa-search-plus"></i></button>     </div>  </div>  <div class="clearfix"></div></div >');
                fileObj.btnClose.click(function () {
                    fileObj.destroy();
                    ////
                    //////
                });
                fileObj.btnVer.click(function () {
                    document.getElementById('kvFileinputModal').classList.remove('fade');
                    //oculto el panel del resultado
                    document.getElementById('kvFileinputModal').style.zIndex = "100";
            //muestro el panel de seleccion de documento firmado
            //document.getElementById('validarFirmaForm').style.display = 'block';
                    ////this.currentItem
                    //////
                    var nodoPadre = document.getElementById('nombreImg');

                    if (nodoPadre.hasChildNodes()) {
                        nodoPadre.removeChild(nodoPadre.lastChild);
                    }
                    //paso 2: crear nodo tipo Text
                    var nodoHIjo = document.createTextNode(fileObj.file.name+' (' + fileSize + ' KB)' );

                    //paso 3: añadir el nodo Text como hijo del Element
                    nodoPadre.appendChild(nodoHIjo);
                    document.getElementById("imagenImg").src = fileObj.contenidoFileB64;
                });
                /////
                callback();
            }
        ...
        initJcrop: function (options, parent, img, appName) {
            ...
            if (options.preview.display) {
                ...
                parent.append('<div class="preview-pane" style="width:' + prevSize[0] + 'px;height:' + prevSize[1] + 'px;"><div class="preview-container" style="width:' + prevSize[0] + 'px;height:' + prevSize[1] + 'px; overflow:hidden"><img src="' + img.src + '" class="jcrop-preview im-prv" /></div></div> ' +
                    '<input type="hidden" class="jcropx" name="' + appName + '-x[]" /><input type="hidden" class="jcropy" name="' + appName + '-y[]" /><input type="hidden" class="jcropw" name="' + appName + '-w[]" /><input type="hidden" class="jcroph" name="' + appName + '-h[]" />');
                    ...
                    if ($(selector[k]).is('form')) {
                        $(selector[k]).each(function () {
                            elements.push({
                                type: "form",
                                obj: $(this),
                                pseudoChild: (el[i].form[0] === $(this)[0])
                            });
                        });
                    }
                ...
                for (j = 0; j < elements.length; j++) {
                    ...
                    switch (elements[i].type) {
                        case "form":
                            ...
                            if (el === elements[j].obj[0] || (elements[j].type === "pseudoinput" && el == elements[j].obj.form[0]) || (elements[j].type === "input" && el == elements[j].obj.parents('form')[0])) {
            ...
            if (typeof (elements) === 'string') {
                ...
                for (j = 0; j < elements.length; j++) {
                    ...
                    switch (elem.type) {
                        ...
                        case "form":
                            $.each(elem.obj.find(':input'), function () {
                                if ($(this).not(':submit')) formData.append($(this).attr("name"), $(this).val());
                            });
                            var app = elem.obj.data("appCustomFile");
                            if (typeof (app) == "undefined") {
                                elem.obj.data("appCustomFile", []);
